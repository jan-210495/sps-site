...
(function() {
  ...
  const i18n = {
    ...
    ar: {
      ...
      // Home
      'home.hero.title': 'فوج مار أفرام السرياني البطريركي – دمشق',
      'home.hero.subtitle': 'إيمان وخدمة وأخوّة في قلب دمشق',
  ...
  const getLang = () => (localStorage.getItem('lang') === 'ar' ? 'ar' : 'en');
  ...
  const applyI18n = (lang) => {
    ...
    document.querySelectorAll('[data-lang]')?.forEach(btn => {
      ...
      if (is) {
        btn.classList.remove('btn-outline-light', 'btn-light', 'btn-gold');
        btn.classList.add(lang === 'ar' ? 'btn-gold' : 'btn-light');
      } else {
        btn.classList.remove('btn-light', 'btn-gold');
